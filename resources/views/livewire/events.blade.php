    ...
    <div>
        <div class="w-full flex">
            @auth
                ...
                <button wire:click="addTopic" class="w-full md:w-1/6 bg-stl-red hover:opacity-75 rounded-r-full focus:outline-none focus:opacity-75">Add New Topic</button>
            @else
        ...
        <div class="w-full flex mt-8">
            <button class="bg-blue-800 p-2 rounded-lg flex-1 mx-3 focus:outline-none uppercase font-bold focus:opacity-75" wire:click="$toggle('hideUnscheduled')"><i class="far {{$hideUnscheduled ? 'fa-check-square' : 'fa-square'}} mr-1"></i> Hide Unscheduled Events</button>
            <button class="bg-blue-800 p-2 rounded-lg flex-1 mx-3 focus:outline-none uppercase font-bold focus:opacity-75" wire:click="$toggle('hideScheduled')"><i class="far {{$hideScheduled ? 'fa-check-square' : 'fa-square'}} mr-1"></i> Hide Scheduled Events</button>
            <button class="bg-blue-800 p-2 rounded-lg flex-1 mx-3 focus:outline-none uppercase font-bold focus:opacity-75" wire:click="$toggle('hideCompleted')"><i class="far {{$hideCompleted ? 'fa-check-square' : 'fa-square'}} mr-1"></i> Hide Completed Events</button>
            ...
            @foreach($events as $event)
                <div class="w-full flex flex-wrap border-b-2 border-gray-200 py-4">
                    <div class="w-1/12 text-center text-2xl">
                        <button><i class="fas fa-arrow-circle-up cursor-pointer text-blue-800 hover:text-blue-600"></i></button>
                        <div class="text-4xl">{{$event->score}}</div>
                        <button><i class="fas fa-arrow-circle-down cursor-pointer text-blue-800 hover:text-blue-600"></i></button>
                    </div>
